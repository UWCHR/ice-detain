---
title: "FY11-24YTD ICE detention analysis"
author: "UWCHR"
date: "2024-02-14"
output:
    html_document:
...
df <- read_delim(here::here('write', 'input', 'ice_detentions_fy11-24ytd.csv.gz'),
                 delim = '|',
                 col_types = cols(
...
This dataset includes `r nrow(df)` ICE detention placement records after dropping duplicate records and records missing `anonymized_id` values[^1], representing `r length(unique(df$anonymized_identifier))` unique individuals and `r length(unique(df$detention_facility))` unique detention facilities.
...
Data was released by ICE in separate annual spreadsheets containing all detention placement records for detention stays with an initial book-in during that U.S. government fiscal year. Stay records spanning multiple fiscal years do not appear to be duplicated in successive datasets. Within each spreadsheet, detention histories are sorted by `anonymized_identifier` and `detention_book_in_date_and_time`, resulting in "blocs" of records for each stay.
...
See below for a representative bloc of detention placement records pertaining to a single completed individual detention stay:
...
headcount <- read_delim(here::here('write', 'input', 'headcount_fy11-24ytd.csv.gz'), delim='|')
